...
test("loads Spanish home at /", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1 })).toHaveText("Lo importante de la IA, en español");
  await expect(page.locator("html")).toHaveAttribute("lang", "es");
});

test("loads English home at /en", async ({ page }) => {
  await page.goto("/en");
  await expect(page.getByRole("heading", { level: 1 })).toHaveText("The AI news that matters");
